...
export default {
  ...
  methods: {
    ...
    handleDblClick (key) {
      const targetKeys = this.targetKeys
      const index = this.leftData.map(data => data.key).indexOf(key)
      const direction = index > -1 ? 'right' : 'left'
      const moveKeys = [key]
      const newTargetKeys = direction === 'right'
        ? moveKeys.concat(targetKeys)
        : targetKeys.filter(targetKeys => targetKeys !== key)
      this.$emit('on-dblclick', newTargetKeys, direction, moveKeys)
    }
  },
  render (h) {
